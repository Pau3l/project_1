  ...
  return (
    <Dialog.Root open={open} onOpenChange={(val) => {
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[550px] rounded-2xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] z-[70] overflow-hidden focus:outline-none flex flex-col backdrop-blur-2xl animate-in zoom-in-95 fade-in duration-300 ${
          isDark ? 'bg-[#0d0d0d]/95 border border-white/[0.08]' : 'bg-white/95 border border-black/[0.08]'
        }`}>
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.06] bg-white/[0.02] shrink-0">
          ...
          {/* Footer */}
          <div className="p-6 border-t border-white/[0.06] bg-white/[0.01] flex gap-3">
            <button 
              onClick={onReset}
            ...
            <Dialog.Close asChild>
              <button className="flex-1 bg-[#ff4d00] hover:bg-[#ff5d14] text-white font-black py-3 rounded-xl text-xs uppercase tracking-widest shadow-[0_10px_20px_-5px_rgba(255,77,0,0.35)] active:scale-[0.98] relative overflow-hidden group">
